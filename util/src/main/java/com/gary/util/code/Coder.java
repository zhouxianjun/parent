...
public abstract class Coder {
	 ...
	 * @return 中文字符串
	 */
	public static String decodeUnicode(String s, String code) {
		String result = "";
		try {
			String temp = "";
			if (s == null)
				return "";
			if (s.length() % 6 != 0)// 一般来说字符串有误
				return "";
			// 好像不用考虑是否int不够的问题，s.length()返回的就是int...
			// 欢迎大牛拍砖
			for (int i = 0; i < s.length() / 6; i++) {
				temp = s.substring(6 * i, 6 * (i + 1));
				result = result + java.net.URLDecoder.decode(temp, code);
			}
			return result;
		} catch (Exception e) {
			return "";
	 ...
	 * @return 字符串
	 */
	public String binary2String(String binaryString) {
	 ...
	 * @return
	 */
	public static String netUnicode(String str) {
		return escape(str).replaceAll("%u", "&#x");
	}
	 ...
	 * @return
	 */
	public static String unNetUnicode(String str) {
		return unescape(str.replaceAll("&#x", "%u"));
	}
