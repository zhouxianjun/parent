...
public class ClassPathScanHandler {
    ...
    public Set<Class<?>> heatLoadClasses(String basePackage, String path, File file) throws MalformedURLException{
    	Set<Class<?>> classes = new LinkedHashSet<Class<?>>();
    	if(file.getName().endsWith(".jar")){
    		String package2Path = basePackage.replace('.', '/');
    		String url = "jar:file:/" + file.getAbsolutePath() + "!/" + package2Path;
    		logger.info("扫描jar文件中的类....");
            doScanPackageClassesByJar(basePackage, new URL(url), true, classes);
    	}
    	return classes;
    }
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (this.filterClassName(classSimpleName)) {
                    ...
                    try {
                    	//classes.add(hscl.loadClass(className, entry.getTime(), jar.getInputStream(entry), entry.getSize()));
    ...
    private void doScanPackageClassesByFile(Set<Class<?>> classes,
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                ...
                try {
                	//classes.add(hscl.loadClass(packageName + '.' + className, file.lastModified(), new FileInputStream(file), file.length()));
